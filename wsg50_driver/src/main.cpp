...
GripperActionServer::GripperActionServer(const rclcpp::NodeOptions &options) : Node("gripper_server_node", options)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Connecting to Gripper with IP addr: %s and Port: %d.", this->addr_.c_str(),
                this->port_);
    ...
    this->gripper_command_server_ = rclcpp_action::create_server<GripperCommand>(
        this, "~/gripper_action", std::bind(&GripperActionServer::handle_goal, this, _1, _2),
    ...
    if (gripper_->connected)
    {
        ...
        rclcpp::sleep_for(5s);
        if (grasp_force_ > 0)
            gripper_->setGraspingForceLimit(grasp_force_);
        this->sub_ack_ = this->create_subscription<std_msgs::msg::Bool>(
            "~/ack_gripper", 10, std::bind(&GripperActionServer::ackCallback, this, _1));
    }
}
...
GripperActionServer::handel_cancel(const std::shared_ptr<GoalHandleGripperCommand> goal_handle)
{
...
void GripperActionServer::onExecuteGripperCommand(const std::shared_ptr<GoalHandleGripperCommand> &goal_handle)
{
    ...
    {
        RCLCPP_ERROR(this->get_logger(), "GripperServer: Commanding out of range width! max_width = %f, command = %f ",
                     GRIPPER_MAX_OPEN, targetWidth);
...
bool GripperActionServer::gripper_command(const double targetWidth, const double targetForce)
{
    ...
    else
    {
        ...
        while (!res)
            res = gripper_->move(targetWidth, 40.0, false, false);
        return res;
    }
}
